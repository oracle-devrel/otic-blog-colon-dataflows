...
public class OciGlobber {
  public static final Logger LOG =
      LoggerFactory.getLogger(OciGlobber.class.getName());
  ...
  public OciGlobber(FileSystem fs, Path pathPattern, PathFilter filter) {
    this.fs = fs;
    this.fc = null;
  ...
  /*
  public OciGlobber(FileContext fc, Path pathPattern, PathFilter filter) {
    this.fs = null;
    this.fc = fc;
    this.pathPattern = pathPattern;
    this.filter = filter;
  ...
  */
  ...
  private Path fixRelativePartOci(Path path) {
  ...
  private FileStatus[] doGlob() throws IOException {
    ...
    // System.out.println("Authority is " + scheme);
    ...
    // System.out.println("pathPatternString is " + pathPatternString);
    ...
    for (String flatPattern : flattenedPatterns) {
      ...
      Path absPattern = fixRelativePartOci(new Path(
          flatPattern.isEmpty() ? Path.CUR_DIR : flatPattern));
      // System.out.println("absPattern is " + absPattern);
      ...
      for (int componentIdx = 0; componentIdx < components.size();
        ...
        for (FileStatus candidate : candidates) {
          // System.out.println("globFilter is " + globFilter);
          // System.out.println("globFilter has pattern? " + globFilter.hasPattern());
          if (globFilter.hasPattern()) {
            ...
            for (FileStatus child : children) {
              ...
              // XXX: Changed, seems to not modify anything.
            ...
            // System.out.println("no filter candidate is " + candidate);
            // System.out.println("no filter component " + component);
            // XXX
            //FileStatus childStatus = getFileStatus(
            //    new Path(candidate.getPath(), component));
            String newPath = candidate.getPath().toString() + '/' + component;
            // System.out.println("no filter newPath became " + newPath);
            FileStatus childStatus = getFileStatus(new Path(newPath));
            if (childStatus != null) {
